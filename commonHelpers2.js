import"./assets/styles-9f48612f.js";import{a as v,P as V}from"./assets/vendor-d107dae6.js";const f="https://food-boutique.b.goit.study/api/",y=async t=>{let e=`${f}products/?`;for(const s in t)e+=`&${s}=${t[s].toString()}`;const a=await(await v.get(e)).data;return localStorage.setItem("fetched-products",JSON.stringify(a.results)),a},W=async()=>{const t=await v.get(`${f}products/categories/`),e=await t.data;return localStorage.setItem("product-categories",JSON.stringify(e)),await t.data},K=async()=>await(await v.get(`${f}products/discount/`)).data,G=async(t=5)=>{const e=Number.isInteger(t)?`${f}products/popular/?limit=${t}`:`${f}products/popular/`;return await(await v.get(e)).data},X=async t=>await(await v.get(`${f}products/${t}`)).data,Y=async t=>await(await v.post(`${f}subscription/`,{email:`${t}`})).data,q={divProdCards:document.querySelector(".js-product-cards")};function h(t){q.divProdCards.innerHTML="",q.divProdCards.insertAdjacentHTML("afterbegin",Z(t))}function Z(t){return t.results.map(({_id:e,name:o,img:a,category:s,price:u,size:p,popularity:g})=>`<div class="product-card-prod" id="${e}">
          ...
          <div class="aside-product-card" id="${e}">
              ...
              </div>`).join("")}async function dt(){const t=await K();k.discountProductCards.insertAdjacentHTML("beforeend",ut(t))}function ut(t){return t.slice(0,2).map(({_id:o,name:a,img:s,price:u})=>`<div class="container-for-discount-items">
